...
## Repository surfaces (Desk, Shelf, Reader)
...
After release, the two copies are independent until the next release. Normal
manuscript flow is **Desk → Shelf**; do not create a two-way sync.

Current authoring instances use `role: "desk"`.

### Desk

The **Desk** is the authoring instance. Working manuscripts, research,
experiments, media, editions, and the next revision of published books live
here. Desk owns:

- `books/<slug>/**` — the working publication: manuscript, `research/`,
  `media/`, and any `editions/` intent files (this is draft and next-edition
  state, not released snapshot truth)
- root `README.md` `## The books` — the authoring catalog table the local
  Publishing Desk discovers
- `catalog.json` — the Desk's working inventory
- `imprint.json` — Desk identity
- local Reader and the `desk/` Publishing Desk application, updated through the
  instance-specific local sync contract; authoring/release tooling

Desk rules:

- Keep research current with the manuscript while drafting. On Desk, research
  may move ahead of the released edition.
- A Desk is private by default, but may deliberately be public or lower-profile.
  Anything committed to a public Desk is public working history even before
  release.

### Shelf

The **Shelf** is the public publishing instance and canonical promoted release
surface. It owns deliberately released snapshots, the public catalog, identity,
release provenance, and the visible web shelf. Shelf owns:

- `books/<slug>/**` — the released manuscript snapshots (the canonical source
  for every derived surface)
- `catalog.json` — machine-readable inventory (single source of truth for
  "published"); `check-catalog.py` validates it against `books/`
- root `README.md` — the human `## The books` table (single source for the
  visible web shelf)
- `imprint.json` — Shelf identity
- Shelf-owned Reader integration files, shell, service worker, and styles
- release provenance (`books/<slug>/release.json`) and this repository's
  history

A released book appears on several generated public surfaces. Keep them
consistent with `books/<slug>/` and `catalog.json`; the rest are derived.

| Surface | Purpose | Regenerated by |
|---|---|---|
| `books/<slug>/**` | the released manuscript snapshot (canonical source) | a release from Desk |
| `catalog.json` | machine-readable inventory (source of truth for "published") | `check-catalog.py` validates it against `books/` |
| root `README.md` | the human `## The books` table | the release command |
| `llms.txt` | machine-readable reading list | hand-maintained; keep in sync with `catalog.json` |
| `publication/<slug>/index.html` + `cover.svg` | per-publication crawlable landing page | `generate-publication-pages.py` |
| `publication/index.html` | gallery of all publications | `generate-publication-pages.py` |
| `sitemap.xml` | search discovery for every `publication/` page | `generate-publication-pages.py` |
| `reader/…` | the Reader UI | `scripts/sync-ui.sh --shelf-safe`, from Bookself only |
...
- Every publication that is listed must be `Published`; unlisting (removing a
  slug) is the Shelf way to hide something, and it is never retroactive in Git
  history.
- `llms.txt` is the one surface not produced by a script; keep its released
  list identical to `catalog.json` and do not let it drift.
- Run `check-catalog.py` and the equivalent Reader-link check after any catalog
  or book-list change. When CI reports a surface mismatch, fix the surface, not
  the checker.
...
pipeline, or a scheduled re-builder that commits content. Custom Shelf
workflows may verify invariants, but they must never rewrite released content.
The release path works with zero GitHub Actions minutes; publication changes
are committed deliberately through normal Git operations, not by scheduled
bots.

The single write-capable exception is the Reader sync: a workflow that streams
`reader/**` from Bookself through `scripts/sync-ui.sh --shelf-safe` and nothing
else. It is the exception that proves the read-only rule. Never widen it, and
never grant `contents: write` or `permissions: write-all` to any other
repository workflow.

The release command does not write `llms.txt`; add a released publication there
in the same change that lands the release.

Instance-owned files are never overwritten by framework sync: `books/`, root
`README.md`, `catalog.json`, `imprint.json`, instance-specific
collaboration/configuration files, and Shelf-owned Reader integration files
protected by the `--shelf-safe` contract.

A Shelf must never receive Bookself's `desk/` tree and must never execute
Reader code from the Bookself Pages deployment at runtime. Unsafe whole-tree
sync into a destination whose imprint role is `shelf` must fail before
mutation.

### Reader

The **Reader** is reusable Bookself software copied into both roles, shared by
Desk proofs and Shelf releases. It is not a fourth source of publication truth:
publication content lives under `books/<slug>/`, instance identity lives in
`imprint.json`, and the Reader only presents them.
...
integration boundary and publication state. Framework sync requires explicit
...
- A Desk update copies shared Reader and Desk framework modules while Desk-owned
  shell, identity, adapters, manuscripts, and catalog/release state remain
  instance data.
- A `--shelf-safe` update preserves the Shelf-owned Reader shell, service
  worker, adapter, identity styles, and publication state, and materializes
  upstream `reader/js/app.js` locally as `reader/js/app-core.js`.
...
- See **Media**, **Citations and references**, and **LaTeX math** below for
  figures, footnotes and citations, and mathematical notation.
...
Media is first-class publication content with provenance, rights, and
production consequences, not decorative attachment.
...
- The intended asset pipeline is `source asset → provenance + rights + intent →
  publication asset → optimized/formatted derivatives → Reader / digital
  edition / print edition`. Preserve rebuildable source media and derive
  platform-specific files deterministically where possible.
- Every figure needs an alt description; a non-visual reader must not lose the
  point of the image.
- Keep a high-quality RGB master for cover artwork; derive printer-specific
  CMYK PDFs without destroying the source.
- See `docs/covers-and-editions.md` for cover/edition production and
  `docs/publication-formats.md` for figures across formats.
...
- See `docs/academic-writing.md` for the scholarly Markdown conventions and
  `docs/latex.md` for the boundaries of the optional full-TeX workflow.
...
introduce document classes, package installation, `.bib` parsing, BibTeX/Biber,
CSL, TikZ, or a required build step unless the task explicitly advances the
optional full-TeX workflow. See `docs/latex.md` for mathematical notation and
`docs/research.md` for publication provenance.

## Publishing production

Production turns a released work into distributor, print, and digital editions
while keeping the source as canonical truth.

- Edition intent files live under `books/<slug>/editions/` and describe intent
  (trim, stock, target family) rather than printer-generated coordinates.
- Exports are disposable delivery artifacts: `.epub`, interior PDF, paperback
  or hardcover cover PDF, JPEG storefront cover, or another printer/distributor
  file. Regenerating an export never replaces canonical source.
- **The cover rule:** create high-resolution source artwork without baked-in
  title, subtitle, author name, logo, price, barcode, trim marks, spine copy,
  or printer guides. Keep important subjects away from the edges and leave
  intentional negative space where Bookself composes the title, author, spine,
  bleed, safe zones, and wrap geometry deterministically from edition data.
- Source resolution should sit comfortably above the final print target
  (commonly 300 ppi). Do not rely on DPI metadata alone; pixel dimensions
  divided by intended physical size determine effective print resolution.
- When Bookself presets and a live printer specification (for example Amazon
...
- Do not add a scheduled re-builder that commits publication content, or any new
  repository workflow with write permission. On a Shelf, the Reader sync from
  Bookself is the only write-capable workflow, and it must never be widened.
- Do not let a hand-maintained public surface such as `llms.txt` drift from
  `catalog.json`. When CI flags a surface mismatch, fix the surface, not the
  checker.
...
`Published`, one root README row under **The books**, an entry in `catalog.json`,
a release provenance record (`books/<slug>/release.json`), a `llms.txt` entry,
and (for the web shelf) generated rows in `publication/` and `sitemap.xml`.
Normally the Release command prepares most of these together, but it does not
write `llms.txt`; add the released slug there in the same change. Do not change
only one side.

**Unpublish.** On the Shelf, set Status to anything except `Published` and
remove the root catalog row. At the same time remove the slug from
`catalog.json`, the root README `## The books` table, and the `llms.txt`
released list, and (if present) its `publication/` directory. Remember that
removing current files does not make content already pushed to public Git
history private.
