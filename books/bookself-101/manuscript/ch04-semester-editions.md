...
Bookself's Binder and Shelf model is a way to keep both.
...
During the fall, her private Binder keeps moving. She rewrites an explanation after a difficult class discussion. She saves a stronger map for next year. A teaching assistant proposes a new primary-source example. None of those changes have to alter what the current students see immediately.

**Key terms**

| Term | Meaning in this course |
|---|---|
| **Working edition** | The private Binder copy being drafted or revised for a future release. |
| **Released edition** | The committed public Shelf snapshot readers are meant to use. |
| **Release** | The deliberate operation that prepares a verified copy of a committed Binder publication for the Shelf. |
...
The normal release begins only after the Binder version has been committed. From the private Binder checkout, an instructor can prepare the Shelf replacement with:

```sh
scripts/release-book.sh bookself-101 ../shelf
```

The command runs locally. It checks that the source is a Binder and the destination is a Shelf, refuses relevant uncommitted changes, copies the publication snapshot, sets the Shelf copy to `Published`, updates the public catalog, verifies the copied publication, and stops before commit or push.

That final pause is important. Release preparation is not the same thing as publication. The instructor can inspect the public diff before deciding to commit and push it. A department can use a pull request for that review if it wants one; an individual instructor can use an ordinary local Git review. Bookself does not require a private-repository GitHub Actions job to move the book from Binder to Shelf.
...
Dr. Rivera teaches the fall edition from September through December. She collects revisions in Binder during those months but leaves the assigned Shelf text stable except for two documented factual corrections. In January she reviews the Binder changes, commits the new course-text state, runs the local release command, inspects the Shelf diff, and publishes the spring edition. The fall version remains recoverable in Shelf Git history even though the Reader now opens the spring text by default.
