...
The private Binder is the working room. Drafts, experiments, and the next edition of a published book belong there. The public Shelf is different: it holds the edition readers are supposed to be reading now. An agent should not quietly turn the Shelf into a drafting workspace because it happens to be convenient to edit a public repository.

So a normal session is almost boring. Pick the book in Binder. Read enough surrounding prose to hear its voice. Change the chapter that was asked for and leave the neighboring chapters alone. Commit the work. Serve the Binder locally and open the Reader if the prose needs to be proofed as pages rather than as source text.
...
When the manuscript is ready to become the next public edition, the agent does not copy and paste a few files by intuition. The Binder publication is committed first. Then `scripts/release-book.sh` prepares the Shelf copy locally. It checks that the source and destination are the right kinds of Bookself instance, refuses dirty release paths, replaces the public publication snapshot, changes the Shelf copy to `Published`, updates the catalog, and verifies the copied files. It stops before commit or push so a human can still read the diff.
...
The agent, in other words, is another pair of hands in the Binder. The book is still the files.
