...
That distinction is the reason Bookself has both a Desk and a Shelf. The Desk can contain the awkward middle of a book: abandoned paragraphs, half-rewritten chapters, notes that only make sense this week. The Shelf should be calmer. It holds the edition that readers are meant to encounter while the next one is still being worked out somewhere private.

The repositories do not point at each other. Shelf does not mount Desk, fetch from it in the browser, or hold a magic reference to private files. When an edition is ready, Bookself copies the committed publication from Desk into a local Shelf checkout. After that, the public copy has its own Git history and can remain unchanged for as long as the next revision takes.

The normal release begins with a commit on the Desk. That matters because “whatever happens to be on this laptop right now” is a poor definition of an edition. The release command refuses uncommitted publication changes, so the source of a public release is an actual Git snapshot that can be found again.

Then run `scripts/release-book.sh your-slug ../shelf` from the Desk. The command verifies that it is moving in the right direction, from a Desk to a Shelf. It checks that the Shelf files it is about to replace are clean. It stages the new publication, changes only the Shelf copy's status to `Published`, adds or updates the public catalog row, and verifies that the publication files match the committed Desk snapshot. If something goes wrong during replacement, it restores the previous Shelf files rather than leaving half an edition behind.
