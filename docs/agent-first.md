...
> Use Bookself to create a private writing space and a public shelf for me. Start my first book from this idea, choose a good reading style, validate it, and publish the first finished version to my shelf.
...
- a private Binder for unpublished work;
...
For example, if the user says **“write my first book in my Binder and publish it to my Shelf”**, that is explicit intent to:

1. create or locate the Binder and Shelf;
...
7. commit the release source in Binder;
...
Then work primarily in the Binder until the user has asked for a public release.
...
- create a private Binder repository and a public Shelf repository when repository-creation capability exists;
- copy/stamp the platform files according to `scripts/stamp-instance.py` semantics;
- preserve `reader/` and `desk/` as shared UI;
- keep `books/`, root `README.md`, and `imprint.json` instance-owned;
- make manuscript changes in Binder;
- create a Shelf release as an independent snapshot rather than a live reference to Binder.
...
- Private Binder created at `owner/my-book-binder`.
...
- Binder release source committed at `<sha>`.
...
The visual Studio, templates, scripts, docs, Reader, and Desk make the protocol:
