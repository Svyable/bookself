...
function saveMode(mode) {
  try {
    ...
    // Storage can be unavailable in hardened/private contexts; the guide still works for this session.
  }
}
...
function installStyles() {
  ...
  style.textContent = `
    .reading-guide {
      ...
      inset: 0 0 auto 0;
      width: 100vw;
      height: var(--reading-guide-height, 3rem);
      pointer-events: none;
      opacity: 0;
      transform: translate3d(0, var(--reading-guide-y, 50vh), 0);
      border-block: 1px solid color-mix(in srgb, currentColor 18%, transparent);
      background: color-mix(in srgb, Canvas 8%, transparent);
      box-shadow:
        0 -100vh 0 100vh rgba(18, 17, 14, .16),
        0 100vh 0 100vh rgba(18, 17, 14, .16);
    ...
    [data-theme="night"] .reading-guide {
      box-shadow:
        0 -100vh 0 100vh rgba(0, 0, 0, .28),
        0 100vh 0 100vh rgba(0, 0, 0, .28);
    }
    .reading-guide-control {
      margin-block-start: .9rem;
      padding-block-start: .8rem;
      border-block-start: 1px solid color-mix(in srgb, currentColor 14%, transparent);
    }
    .reading-guide-control legend {
      font: inherit;
      font-weight: 650;
      padding: 0;
    ...
    .reading-guide-options button[aria-pressed="true"] {
...
function installControl(onMode) {
  ...
  fieldset.addEventListener('click', (event) => {
    ...
    if (!button) return;
    onMode(button.dataset.guideMode);
  });
  return fieldset;
}

function activeOverlays() {
  return OVERLAY_IDS.some((id) => document.getElementById(id)?.classList.contains('active'));
}
...
export function installReadingGuide(doc = document) {
  ...
  function updateButtons() {
    fieldset?.querySelectorAll('[data-guide-mode]').forEach((button) => {
  ...
  function render() {
    ...
    const visible = guideVisible({
      mode,
      stage: doc.body.dataset.stage,
      overlaysOpen: activeOverlays(),
  ...
  function setMode(next) {
    mode = normalizeGuideMode(next);
    saveMode(mode);
    render();
  }

  function cycleMode() {
    const order = ['off', 'line', 'band'];
  ...
  doc.addEventListener('pointermove', (event) => {
    if (mode === 'off' || !shouldTrackPointer(event.pointerType)) return;
    if (!event.target.closest?.(CONTENT_SELECTOR)) return;
    lineHeight = targetLineHeight(event.target);
    y = event.clientY;
    render();
  }, { passive: true });
  ...
  doc.addEventListener('focusin', (event) => {
    if (mode === 'off') return;
    const content = event.target.closest?.(CONTENT_SELECTOR);
    if (!content) return;
    const rect = event.target.getBoundingClientRect?.();
    lineHeight = targetLineHeight(event.target);
    if (rect?.height) y = rect.top + Math.min(rect.height / 2, lineHeight * 1.6);
    render();
  });
  ...
  doc.addEventListener('keydown', (event) => {
    ...
    event.preventDefault();
    y = nextGuideY(y, event.key === 'ArrowDown' ? 1 : -1, viewportHeight(), guideHeight(mode, lineHeight));
    render();
  });
  ...
  for (const id of OVERLAY_IDS) {
    ...
    if (node) observer.observe(node, { attributes: true, attributeFilter: ['class', 'hidden'] });
  }
  ...
  return Object.freeze({
    ...
    destroy() {
      ...
      guide.remove();
      fieldset?.remove();
      delete doc.body.dataset.readingGuideInstalled;
    },
  });
}
